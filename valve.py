...
class Valve:
    ...
    def xi_t(self,time):#rate of opening and closure 
        delta_t=0.001
        t=[time,time+delta_t]
    ...
    def bernoulli_resistance(self,time):#B g*cm^(-1)*s^(-2)
        Aeff=self.Aeff(time)
        return self.rho/(2*Aeff**2)

    def blood_inertance(self,time):#L g*cm^(-4)
        Aeff=self.Aeff(time)
        return (self.rho*self.leff)/Aeff
       
    def delta_p(self,q,diff_q,time):
        self.q=q
        self.diff_q=diff_q
        B=self.bernoulli_resistance(time)
        L=self.blood_inertance(time)
